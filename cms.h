    ...
    if (anonymous) {
        ...
        if (!is_new) {
            if (!cms_validate_header((CmsHeader *)base, (uint64_t)st.st_size)) {
                 ...
                 * ftruncate and cms_init_header below leaves a full-size, all-zero
 ...
 * The counter matrix + geometry are immutable in a sealed file, so estimate /
 * total / stats read directly with no reader-slot / rwlock traffic -- the
